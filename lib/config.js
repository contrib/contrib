...
config.normalize = function(configObj, isTop){
  var newConfig = {};
  ...
  _.forEach(configObj, function(val, key){
    ...
    if (typeof val === 'object') {
      newConfig[key] = config.normalize(val, false);
      return;
    }
...
config.isSingleCommandConfig = function(cmdConfig){
  return typeof cmdConfig === 'object' && (cmdConfig.steps || Array.isArray(cmdConfig));
};
...
config.example = {
  ...
  feature: {
    ...
    start: {
      steps: [
        {
          desc: 'First step for creating a feature',
          exec: ''
        }
      ]
    },
